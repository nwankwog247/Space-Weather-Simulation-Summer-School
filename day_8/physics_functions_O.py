...
# -*- coding: utf-8 -*-
"""
Created on Wed Jul 27 12:40:31 2022
...
cKB_ = 1.38064852e-23 #Boltzmann constant
cRE_ = 6371.0 # earth radius
cG_ = 9.81 #gravity constant
cAMU_ = 1.6726219e-27 #atomic mass unit
cH_ = 6.6261e-34 # planck constant
cC_ = 2.9979e8 #speed of light
cE_ = 1.6022e-19 # one electron charge density
cD2R_ = 3.1415927 / 180.0 #convert from degree to radians
...
def calc_tau(SZA_in_deg, density_in_m3, scale_height_in_km, cross_section):
    ...
    # calculate Tau: i changed to all 37 wavelengths
...
def calculate_Qeuv(density_in_m3,
    ...
    for iWave in range(nWaves): #i changed to all 37 wavelengths
...
def calc_rho(density_o2, mass_o2):
...
def calc_rho(density_n2, mass_n2):
...
def convert_Q_to_dTdt(Qeuv, rho_o2, cp):
    dTdt = Qeuv / (rho_o2 * cp)
    return dTdt

#-----------------------------------------------------------------------------

def convert_Q_to_dTdt(Qeuv, rho_n2, cp):
    dTdt = Qeuv / (rho_n2 * cp)
    return dTdt
